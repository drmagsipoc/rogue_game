...
fn use_item(tcod: &mut Tcod, inventory_id: usize, inventory: &mut Vec<Object>, objects: &mut [Object],
    ...
    if let Some(item) = inventory[inventory_id].item {
        let on_use: fn(&mut Tcod, usize, &mut [Object], &mut Messages, &mut Map) -> UseResult = match item {
            Heal => cast_heal,
            Lightning => cast_lightning,
            Fireball => cast_fireball,
            Confusion => cast_confuse,
        };
        match on_use(tcod, inventory_id, objects, messages, map) {
            UseResult::UsedUp => {
...
fn cast_confuse(tcod: &mut Tcod, _inventory_id: usize, objects: &mut [Object],
    ...
    if let Some(monster_id) = monster_id {
        ...
        message(messages, "No enemy is close enough", colors::RED);
        UseResult::Cancelled
    }
}
...
fn ai_take_turn(monster_id: usize, map: &Map, objects: &mut [Object],
    ...
    if let Some(ai) = objects[monster_id].ai.take() {
        let new_ai = match ai {
            ...
            Confused{previous_ai, num_turns} => ai_confused(monster_id, map,
                                    objects, messages, previous_ai, num_turns)
        };
...
fn ai_confused(monster_id: usize, map: &Map, objects: &mut [Object],
               messages: &mut Messages, previous_ai: Box<Ai>, num_turns: i32)
                    -> Ai {
    // still confused
    if num_turns >= 0 {
...
fn handle_keys(key: Key, tcod: &mut Tcod, objects: &mut Vec<Object>, map: &mut Map,
...
fn main() {
    ...
    // create the list of game messages and their color.
    ...
    // welcome message
    message(&mut messages, "Hello guys, let's play!", colors::RED);
    ...
    while !tcod.root.window_closed() {
        ...
        let player_action = handle_keys(key, &mut tcod, &mut objects, &mut map,
